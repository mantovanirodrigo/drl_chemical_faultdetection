...
def get_files(idv=1, n_train=6000, n_val=2000, w=5, standard=False):
    ...
    for x in range(n_train):
        if (x + 1) % (n_train / 30) == 0:
            print(
                f"... {x+1}/{str(n_train)} files loaded ({int(100*(x+1)/n_train)}%) ..."
            )
    ...
    y_train = train.iloc[:, -1].values.astype(np.float32)

    val = pd.concat(val_files)
    x_val = val.iloc[:, :-1].values.astype(np.float32)
    y_val = val.iloc[:, -1].values.astype(np.float32)
    ...
    x_train = np.array(list(sliding_window(x_train[: 480 * n_train], w)), dtype = np.float32)
    y_train = np.array(list(sliding_window_max(y_train[: 480 * n_train], w)), dtype = np.float32)
    x_val = np.array(list(sliding_window(x_val[: 480 * n_val], w)), dtype = np.float32)
    y_val = np.array(list(sliding_window_max(y_val[: 480 * n_val], w)), dtype = np.float32)
...
def get_far_mdr(model, x, y):
    ...
    _, accuracy_fault = model.evaluate(x_fault, y_fault, verbose = 0)
    _, accuracy_normal = model.evaluate(x_normal, y_normal, verbose = 0)

    FAR = 1 - accuracy_normal
    MDR = 1 - accuracy_fault
